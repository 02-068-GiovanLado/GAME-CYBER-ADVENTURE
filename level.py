...
# from debug import debug # Komentari jika tidak digunakan atau tidak ada file debug.py
from support import *
...
from popup_text import show_opening_popup # <-- PASTIKAN IMPORT INI ADA
...
class Level:
    def __init__(self):
        ...
        # --- AWAL PENAMBAHAN: Flag untuk popup pembuka ---
        ...
        try:
            ...
            except Exception as font_e: # Tangkap exception yang lebih umum jika UI_FONT atau WIN_FONT_SIZE tidak ada
        ...
        self.create_map() # Membuat player dan map

        # Inisialisasi UI dan Upgrade setelah player ada
        self.ui = UI()
        if hasattr(self, 'player') and self.player is not None:
            self.upgrade = Upgrade(self.player)
        else:
            # Jika player tidak ada setelah create_map, ini masalah serius
            print("CRITICAL ERROR: Player tidak ada setelah create_map(). Upgrade menu tidak akan berfungsi.")
            self.upgrade = None # Atau handle error lain
